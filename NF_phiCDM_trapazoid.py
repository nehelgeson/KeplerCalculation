...
for case in range(1, 3, 1):
    ...
    def Li(O, Ok_0):
        l, error = integrate.quad(lambda H0: math.exp((-1/2)*(chi_sq(H0, O, Ok_0))
            + (-1/(2*(s**2)))*((H0 - H0_av)**2)), 
            ...
            #print(O)
               ...
               for b in range (0, int(150/t0), 1):
                   if (O >= Ofunc(b,K)[0]):
                       break
               time2 = time.time()
               print("Loop 0 Time: %.2f millis" % ((time2-time1)*1000))
               ...
               for q in range(0, 8, 1):
                   ...
                   _prev_y = 0
                   _y = 0
                   _loop_check = 1/(1 + z1)*afin
                   first_elem = False
                   for d in range (0, b+1, 1):
                       if (sol[d,2] >= _loop_check):
                           _y = 1/sol[d, 2]
                           r += _y
                           qq += 1
                           qt += 1
                           if first_elem:
                               total += ((_y+_prev_y)/2)*(d-_prev_d)
                           else:
                               first_elem = True
                           _prev_d = d
                           _prev_y = _y
               ...
               time2 = time.time()
               print("Loop 1 Time: %.2f millis" % ((time2-time1)*1000))
               alpha0 = 0
               beta0 = 0

               time1 = time.time()
               print("Len of z_obs: %d" % (len(z_obs)))
               for q in range(0, len(z_obs), 1):
                   for g in range (0, int(150/t0), 1):
                       _temp_bound = afin/(1 + z_obs[q])
                       if (sol[g,2] >= _temp_bound):
               ...
               time2 = time.time()
               print("Loop 2 Time: %.2f millis" % ((time2-time1)*1000))
               O1.append(O)
               a1.append(al)
               Ok1.append(Ok_0)
